...
| `app/components/simulation/TrieScene.tsx` | `kind: 'trie'` — SVG, n-ary layout, edge-এ অক্ষর | Client Component |
...
| `app/lib/simulations/topic10/*.ts` | টপিক ১০-এর ৩টা যাচাই-করা ট্রেস | Data |
...
|---|---|---|
| `array` | `<ArrayScene>` | 1.1 (bar মোড), 1.2, 1.3, 1.4, 1.6, 2.1–2.4, 4.1–4.4, 7.1, 8.4, 9.1–9.3, 9.5, 9.7, 9.10, 9.11, 10.1 |
| `matrix` | `<MatrixScene>` | 1.7 Spiral Matrix, 7.2, 7.3, 8.1, 9.4, 9.6, 9.8, 9.9 |
| `intervals` | `<IntervalsScene>` | 1.5 Merge Intervals |
| `linked-list` | `<LinkedListScene>` | 3.1, 3.2, 3.3, 10.3 |
| `tree` | `<TreeScene>` | 5.1–5.5 |
| `graph` | `<GraphScene>` | 8.2, 8.3, 8.5, 8.6, 8.7 |
| `trie` | `<TrieScene>` | 10.2 |

> সাতটাই — `Scene` union সম্পূর্ণ। নতুন kind লাগলে: নতুন renderer + `SceneView`-এ একটা case, আর কোথাও নয়।
...
- **`bounds` না থাকলে `data-window` বসে না** (`undefined`, `"true"` নয়)। `data-window` `ArrayScene`-এর সাথে ভাগ করা, আর সেখানে `"true"` মানে amber window rail — bounds-হীন গ্রিডের প্রতিটা ঘরে সেটা বসালে গোটা dp টেবিল আন্ডারলাইন হয়ে যেত আর এমন একটা window দাবি করত যা নেই।
- `pointers` না দিলে লেবেলের সারিটাই render হয় না, তাই বাকি scene-গুলোর লেআউট অপরিবর্তিত থাকে।

#### `trie` — দুটো নিয়ম

- **অক্ষর থাকে edge-এ, নোডে নয়।** trie নোড কোনো মান ধরে না — সে **কী বোঝায়** তা ঠিক করে তার কাছে পৌঁছাতে যে অক্ষরগুলো পেরোতে হয়। এই কারণেই `TreeScene` চলত না: তার edge-এ label নেই।
- **`isEnd` = ভেতরের সবুজ বৃত্ত, রং নয়।** নোডের `data-mark` ততক্ষণে walk-এর অবস্থা বলছে; "এখানে শব্দ শেষ" আলাদা তথ্য, তাই আলাদা ভিজ্যুয়াল চ্যানেল।

> layout `TreeScene`-এর মতোই হিসাব করা, কিন্তু n-ary: leaf পরের কলাম নেয়, parent তার সন্তানদের উপরে কেন্দ্রে বসে।
