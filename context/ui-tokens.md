...
| **Simulation** | **`sim-stage` `sim-cell` `sim-index` `sim-cell-value` `sim-subvalue` `sim-pointer` `sim-window-tag` `sim-bar-track` `sim-bar` `sim-bar-fill` `sim-span` `sim-axis` `sim-node` `sim-node-val` `sim-node-link` `sim-link` `sim-loop` `sim-tree-node` `sim-tree-val` `sim-tree-annot` `sim-tree-pointer` `sim-tree-edge` `sim-graph-node` `sim-graph-label` `sim-graph-annot` `sim-graph-pointer` `sim-graph-edge` `sim-graph-arrow` `sim-graph-weight` `sim-trie-node` `sim-trie-end` `sim-trie-edge` `sim-trie-char` `sim-trie-root` `sim-entry` `sim-out` `sim-var` `codeline` `codeline-no` `sim-scrub`** — প্যাটার্ন প্লেয়ার (বিস্তারিত `ui-registry.md` → 🎬 Simulation) |
...
| `data-mark` | `.sim-cell` `.sim-bar` `.sim-span` `.sim-node` `.sim-tree-node` `.sim-graph-node` `.sim-graph-edge` `.sim-graph-arrow` `.sim-trie-node` `.sim-trie-edge` `.sim-entry` | `active` / `done` / `reject` / `fill` |
| `data-on` | `.sim-tree-node` `.sim-tree-edge` | live recursion path-এর উপর |
| `data-dummy` | `.sim-node` | sentinel — ডেটার অংশ নয় |
| `data-end` | `.sim-trie-node` | এই নোডে একটা শব্দ শেষ হয় |
...
- **Simulation:** `sim-stage-*` `sim-cell-*` `sim-active-*` `sim-done-*` `sim-reject-opacity` `sim-fill-*` `sim-window-rail|-width` `sim-index-*` `sim-pointer-*` `sim-bar-*` `sim-node-*` `sim-link-*` `sim-loop-*` `sim-tree-*` `sim-graph-*` `sim-trie-*` `sim-span-*` `sim-axis-border` `codeline-active-*` `codeline-no-*` `sim-scrub-*`
