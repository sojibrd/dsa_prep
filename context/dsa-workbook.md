...
**Statement (Demo):** non-decreasing order-এ sorted একটি integer array `nums` এবং একটি `target` মান দেওয়া আছে। `target` এর starting এবং ending position (index) খুঁজে বের করুন। যদি `target` খুঁজে না পাওয়া যায়, তবে `[-1, -1]` return করুন। runtime complexity অবশ্যই `O(log n)` হতে হবে।
উদাহরণ: `nums = [5,7,7,8,8,10]`, `target = 8` → `[3, 4]` | ⚡ `n ≤ 10⁵`, `-10⁹ ≤ nums[i], target ≤ 10⁹`
...
- [x] **Search Insert Position** — [LC 35](https://leetcode.com/problems/search-insert-position/) — ⚪ Bonus
      → Statement: sorted array `nums` এবং একটি `target` মান দেওয়া আছে। array-তে target খুঁজে পেলে তার index রিটার্ন করুন। আর না পাওয়া গেলে সেটি sorted অর্ডারে রাখলে যে index-এ বসতো, তা রিটার্ন করুন। ওয়ান-পাসে O(log n) টাইমে করতে হবে।
...
- [x] **Binary Search** — [LC 704](https://leetcode.com/problems/binary-search/) — ⚪ Bonus _(টেমপ্লেট মুখস্থ লেখার প্র্যাকটিস)_
      → Statement: ascending order-এ sorted একটি integer array `nums` এবং একটি `target` মান দেওয়া আছে। array-তে `target` খুঁজুন; পাওয়া গেলে তার index এবং না পাওয়া গেলে `-1` রিটার্ন করুন। O(log n) টাইমে সলভ করুন।
...
- [x] **Median of Two Sorted Arrays** — [LC 4](https://leetcode.com/problems/median-of-two-sorted-arrays/) — 🔥 Must-do _(Hard — Google/Apple High; ছোট array-তে partition search)_
      → Statement: `m` এবং `n` সাইজের দুটি sorted array `nums1` এবং `nums2` দেওয়া আছে। এই দুটি array-র overall median (মধ্যমা) O(log(m+n)) সময়ে বের করুন।
      উদাহরণ: `nums1 = [1,3]`, `nums2 = [2]` → `2.00000` | ⚡ `m, n ≤ 1000`, `-10⁶ ≤ nums1[i], nums2[i] ≤ 10⁶`
...
**Statement (Demo):** `n`টি কলার স্তূপ (piles) দেওয়া আছে যেখানে `piles[i]` হলো `i`-তম স্তূপের কলার সংখ্যা। গার্ড আসার আগে `h` ঘণ্টার মধ্যে কোকোকে সব কলা খেতে হবে। প্রতি ঘণ্টায় সে সর্বোচ্চ `k`টি কলা খেতে পারে। যদি কোনো স্তূপে `k` এর চেয়ে কম কলা থাকে, সে সেটি খেয়ে অন্য স্তূপ খাওয়া শুরু করে না (ঐ ঘণ্টা শেষ হওয়া পর্যন্ত অপেক্ষা করে)। কোকোকে সবগুলো কলা `h` ঘণ্টার মধ্যে শেষ করার জন্য ন্যূনতম খাওয়ার গতি `k` (banana-per-hour speed) খুঁজে বের করতে হবে।
উদাহরণ: `piles = [3,6,7,11]`, `h = 8` → `4` | ⚡ `n ≤ 10⁴`, `n ≤ h ≤ 10⁹`, `1 ≤ piles[i] ≤ 10⁹`
...
**Statement (Demo):** non-negative integer array `nums` এবং একটি integer `k` দেওয়া আছে। array-টিকে `k`টি contiguous non-empty subarray-তে ভাগ করতে হবে যাতে এই subarray গুলোর সর্বোচ্চ যোগফল ন্যূনতম (minimize the largest sum among these k subarrays) হয়।
