...
### 🔍 রিভিউ (ফিচার সম্পূর্ণ হওয়ার পর)

engine layer (হুক, types, ৭টা renderer, registry, panel wiring) রিভিউ করা হয়েছে। **দুটো কনট্র্যাক্টই অক্ষুণ্ণ** — Theme Contract (কম্পোনেন্টে কোনো ভিজ্যুয়াল ক্লাস নেই, সব role class + `data-*`) আর Scene কনট্র্যাক্ট (types-এ কোনো স্থানাঙ্ক নেই, তিনটে SVG scene-ই layout হিসাব করে, `SceneView`-ই একমাত্র switch)।

তিনটে ত্রুটি ধরা পড়ে ঠিক করা হয়েছে:

| ফাইল | সমস্যা | সমাধান |
|---|---|---|
| `MatrixScene.tsx` | `bounds` না থাকলেও প্রতিটা ঘরে `data-window="true"` বসত, ফলে **সব dp টেবিলে** amber window rail আঁকা হচ্ছিল (7.2, 7.3, 8.1, 9.4, 9.6, 9.8, 9.9) | `bounds` না থাকলে `undefined` |
| `usePatternSim.ts` | এক-ধাপের টাইমলাইনে `play()` `isPlaying` সেট করত কিন্তু কোনো timer বসত না — transport "Pause"-এ আটকে থাকত | `total <= 1` হলে play কিছু করে না |
| `TrieScene.tsx` | অনুপস্থিত নোডের জন্য `place()` `0` ফেরত দিত, যা parent-এর centering গড়ে মিশে subtree-কে বাঁ প্রান্তে টেনে নিত | `null` ফেরত দিয়ে filter |

প্রথমটা বাস্তবে দৃশ্যমান ছিল; বাকি দুটো latent (কোনো সিমুলেশনে এক-ধাপ নেই, আর কোনো trie ডেটা malformed নয়) — তবু ঠিক করা হয়েছে।

#### ৪৭টা ট্রেস ফাইলের যান্ত্রিক অডিট

রিভিউ ইচ্ছাকৃতভাবে ডেটা ফাইলগুলোর bulk বাদ দিয়েছিল, তাই সেগুলো আলাদা করে script দিয়ে যাচাই করা হয়েছে:

| যাচাই | ফল |
|---|---|
| Scene কনট্র্যাক্ট (`className`, inline `style`, hex রং, স্থানাঙ্ক) | ৪৭/৪৭ পরিষ্কার |
| `highlightLines` workbook fence-এর সীমার মধ্যে | ৪৭/৪৭ পরিষ্কার |
| প্রতিটা `patternId` workbook-এ সত্যিই আছে | ৪৭/৪৭ |
| ডুপ্লিকেট `patternId` বা step `id` | নেই |
| `index.ts`-এ import বাদ পড়া | নেই |

কোনো নতুন ত্রুটি পাওয়া যায়নি।

#### কভারেজ

**অ্যাপ যে ৪৭টা প্যাটার্ন পার্স করে, তার প্রতিটাতেই সিমুলেশন আছে** — `getSimulation()` sparse হিসেবে ডিজাইন করা, কিন্তু বাস্তবে এখন সম্পূর্ণ।

> ⚠️ **টপিক ৬ অনুপস্থিত — ঠিক করা যায়নি।** `dsa-workbook.md`-এর ৭৫ নম্বর লাইনে `@[context/dsa-workbook/6. Heaps / Priority Queues.md]` আছে, কিন্তু ফাইলটা নেই — git history-তেও কখনো ছিল না। parser-এর `fs.existsSync` চেক ব্যর্থ হয়ে লাইনটা literal টেক্সট হিসেবে রেখে দেয়। ফলে অ্যাপ পার্স করে **৯টা টপিক, ৪৭টা প্যাটার্ন, ৮৬টা প্রবলেম**, অথচ workbook-এর উপরের সারাংশ টেবিল দাবি করে **১০, ৫১, ৯১**। ৯.৪-এর মতো এখানে কোনো ডুপ্লিকেট ফাইলে কনটেন্টটা পড়ে নেই — সোর্স কনটেন্ট সত্যিই অস্তিত্বহীন, তাই এটা লিখে না দেওয়া পর্যন্ত ঠিক করার উপায় নেই।

## 🔄 বর্তমানে চলমান

_কিছু নেই — সিমুলেশন ফিচার সম্পূর্ণ ও রিভিউ করা।_
